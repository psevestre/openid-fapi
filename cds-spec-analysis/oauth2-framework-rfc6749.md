...
| <a name="3.2.1"></a>[3.2.1. Client Authentication](https://tools.ietf.org/html/rfc6749#section-3.2.1) | Yes | Yes :stop_sign: | The CDS example payloads do not include a `client_id` in token responses. This disables substitution attack protections. |
| [3.3. Access Token Scope](https://tools.ietf.org/html/rfc6749#section-3.3) | Yes | Unknown :question: :stop_sign: | CDS example payloads do not include `scope` parameters in the request. This clause states *If the client omits the scope parameter when requesting authorization, the authorization server MUST either process the request using a pre-defined default value or fail the reques tindicating an invalid scope.  The authorization server SHOULD document its scope requirements and default value (if defined).* |
...
| [5.1. Successful Response](https://tools.ietf.org/html/rfc6749#section-5.1) | Yes | Yes :stop_sign: | CDS introduces additional claim requirements for token responses including:<li><ul>`expires_in` is **MANDATORY**</ul><ul>`refresh_token_expires_at` is **MANDATORY** for requests intended to be continuous consent requests</ul></li>|
...
| [8.2. Defining New Endpoint Parameters](https://tools.ietf.org/html/rfc6749#section-8.2) | Yes | Yes :stop_sign: | `sharing_expires_at` is introduced but is not registered nor does it follow collision avoidance specifications within this clause |
...
| [10.9. Ensuring Endpoint Authenticity](https://tools.ietf.org/html/rfc6749#section-10.9) | Yes | Yes :stop_sign: | Unclear as to whether all endpoints involved in interaction are secured by TLS certificates that recipients can independently verify (ie. ACCC signed certificates) |
...
| [11. IANA Considerations](https://tools.ietf.org/html/rfc6749#section-11) | Yes | Yes :stop_sign: | Injected claims should be registered via IANA registries |

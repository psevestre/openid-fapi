...
Since OIDF FAPI aims at providing an API protection profile for
...
mechanisms are derived in the Baseline and Advanced profiles.

The ultimate aim is to provide systematic proofs of the security of
the FAPI profiles similar to those in [@arXiv.1901.11520]. Such proofs
...
goals formulated in [@arXiv.1901.11520].
...
## Authorization
FAPI 2.0 profiles MUST ensure that **no attacker can access resources
belonging to a user.**
...
FAPI 2.0 profiles MUST ensure that **no attacker is able to log in at
...
In detail:
  ...
  * For authentication: FAPI 2.0 profiles MUST ensure that **no
    attacker is able to force a user to be logged in under the
    identity of the attacker.**
  * For authorization: FAPI 2.0 profiles MUST ensure that **no
...
arbitrary combinations of the following attackers:
...
from honest ASs and can send users to endpoints of honest ASs.

### A2 - Network attacker

Controls the whole network (like a rogue WiFi access point or a
...
unless the attacker has learned the respective decryption keys. 
...
attacker can read TLS intercepting proxy logs on the RS's side.
